...
public class WikiPediaProcessor implements TextProcessor {

    public HashSet<Triple<String,String, String>> news_relations_triples = new HashSet<>();
    public Map<String,LinkedList<Triple<Integer,Integer,String>>> wordsIndexList = new HashMap();
            ...
            if (word.contains("_")) { // if linguakit merged two (or more) words in NER
                ...
                for (String x : xx){
                    ...
                    if (t==null) {
                        ...
                        if (t==null)
                            throw new Exception(" word not found!! que andas a fazer???");
                    }
    ...
    public Set<Pair<String,String>> getRelevantEntities (){
        ...
        for (String s : news_pos_info.keySet()){
            if (s.matches("N.*")|| s.matches("W") || s.matches("PVALUE") || s.matches("Z")){
